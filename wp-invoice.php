...
define("WP_INVOICE_VERSION_NUM", "2.039");
...
class WP_Invoice {
	...
	function WP_Invoice() {
		...
		// Remove conflicting code by calling a function on WPI overview page
		add_action('admin_head-invoices_page_invoice_settings', array($this, 'admin_head_wpi_specific'));
		add_action('admin_head-toplevel_page_web-invoice_page_overview', array($this, 'admin_head_wpi_specific'));
		add_action('admin_head-invoices_page_new_invoice', array($this, 'admin_head_wpi_specific'));
		...
		add_shortcode('wp-invoice-list', 'wp_invoice_list');
		...
		add_action('wpi_premium_feature_check', array('WPI_Functions', 'feature_check'));
	 ...
	 */
	 function plugin_action_links( $links, $file ){
 		...
 		if ( $file == 'wp-invoice/wp-invoice.php' ){
			$settings_link =  '<a href="'.admin_url("admin.php?page=invoice_settings").'">' . __('Settings') . '</a>';
	...
	/*
		Removes any conflicting js code on wpi pages
	*/
	function admin_head_wpi_specific() {
		remove_action("admin_head","sv_hover_panel_head");
	}
	...
	function wp_invoice_add_pages() {
		...
		invoices_page_new_invoice
		invoices_page_recurring_billing
		invoices_page_invoice_settings
		*/
		...
		$menus['main'] = add_menu_page('Web Invoice System', 'Invoices',  $this->wp_invoice_user_level,'web-invoice_page_overview', array(&$this,'invoice_overview'),$this->uri."/core/images/wp_invoice.png", $_wp_last_object_menu);
		...
		register_column_headers("invoices_page_recurring_billing", $recurring_columns);	

		add_filter('screen_layout_columns', array(&$this, 'on_screen_layout_columns'), 10, 2);		
		
		//Metaboxes
		add_meta_box('wp_invoice_metabox_invoice_details', __('Invoice Details',WP_INVOICE_TRANS_DOMAIN), 'wp_invoice_metabox_invoice_details', 'invoices_page_new_invoice', 'normal', 'high');
		
		if(get_option('wp_invoice_use_recurring') == 'yes')
			add_meta_box('wp_invoice_metabox_recurring_billing', __('Recurring Options',WP_INVOICE_TRANS_DOMAIN), 'wp_invoice_metabox_recurring_billing', 'invoices_page_new_invoice', 'side', 'high');
		
		add_meta_box('wp_invoice_metabox_billing', __('Billing Settings',WP_INVOICE_TRANS_DOMAIN), 'wp_invoice_metabox_billing', 'invoices_page_new_invoice', 'normal', 'core');
		add_meta_box('wp_invoice_metabox_publish', __('Publish',WP_INVOICE_TRANS_DOMAIN), 'wp_invoice_metabox_publish', 'invoices_page_new_invoice', 'side', 'high');
...
*/
	function on_screen_layout_columns($columns, $screen) {
		if ($screen == 'invoices_page_new_invoice') {
			$columns['invoices_page_new_invoice'] = 2;
		}
		return $columns;
	}	
	...
	function admin_print_scripts() {
		...
		wp_enqueue_script('jquery.tablesorter',$this->uri."/core/js/jquery.tablesorter.min.js", array('jquery'));
 		wp_enqueue_script('wp-invoice',$this->uri."/core/js/wp-invoice-2.039.js", array('jquery') );		
...
*/
	...
	function init() {
		...
		function install() {
			...
			wp_schedule_event(time(), 'daily', 'wpi_premium_feature_check');
		...
		// Install default columns
		$user_object =  wp_get_current_user();
		$user_id = $user_object->ID;
		$overview = get_user_option('managetoplevel_page_web-invoice_page_overviewcolumnshidden');
		$recurring = get_user_option('manageinvoices_page_recurring_billingcolumnshidden');

		if(empty($overview)) {
			$defaults[0] = 'user';
			$defaults[1] = 'wp_username';
			$defaults[2] = 'due_date';
			$defaults[3] = '';
			$defaults[4] = '';
			$defaults[5] = '';
		
			update_user_option($user_id, 'managetoplevel_page_web-invoice_page_overviewcolumnshidden', $defaults, true);
		}
		
		if(empty($recurring)) {
			$defaults[0] = 'user';
			$defaults[1] = 'user_email';
			$defaults[2] = 'company_name';
			$defaults[3] = 'date_sent';
			$defaults[4] = '';
			$defaults[5] = '';
			$defaults[6] = '';
			$defaults[7] = '';
		
			update_user_option($user_id, 'manageinvoices_page_recurring_billingcolumnshidden', $defaults, true);
		}
