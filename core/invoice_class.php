	...
	*/
	function wp_invoice_get($invoice_id) {
		...
		foreach($meta_obj as $meta_row) {
			...
			// Convert all string 'true' and 'yes' to boolean true, and negatives to false
			switch($meta_value) {
				case 'yes':
					$meta_value = true;
				break;
		...
		if(!$this->data->user_class->ID): 
			$this->user_deleted = true;
		endif;

		unset($this->data->user_class->user_pass);
		...
		$this->data->id = $this->data->display_id;
		...
		// Determine if invoice is recurring
		if($this->data->subscription_name && $this->data->subscription_unit && $this->data->subscription_total_occurances)
		...
		if($this->data->subscription_start_month && $this->data->subscription_start_year && $this->data->subscription_start_day) {
			$this->data->recurring_auto_start = true; 

			// Nicely formatted recurring start date (expected start date)
			$this->data->recurring_start_date = wp_invoice_Date::convert($this->data->subscription_start_year ."-". $this->data->subscription_start_month ."-". $this->data->subscription_start_day, 'Y-m-d', get_option('date_format'));
			
			// Actual start date set by Authorize.net response, or PayPal subscription response
			//$this->data->subscription_started;
			$this->data->subscription_started_display = wp_invoice_Date::convert($this->data->subscription_started, 'Y-m-d', get_option('date_format'));
			
		} else {
			$this->data->recurring_auto_start = false;
		}
		...
		if(!empty($this->data->subscription_id))
			$this->data->recurring_active = true;
