...
class load_wp_invoice {
	...
	function create_from_template($template_invoice_id, $user_id) {
		...
		$this->wp_invoice_tax = wp_invoice_meta($template_invoice_id,'wp_invoice_tax');
		if($this->wp_invoice_tax == '') $this->wp_invoice_tax = wp_invoice_meta($template_invoice_id,'tax_value');
		...
		$this->gateway_url = $billing_information->display('wp_invoice_gateway_url');
		$this->recurring_gateway_url = $billing_information->display('wp_invoice_recurring_gateway_url');

		$this->moneybookers_allow = $billing_information->display('wp_invoice_moneybookers_allow'); 
	...
	function load_existing($invoice_id = false) {
		...
		$this->tax = wp_invoice_meta($invoice_id,'wp_invoice_tax');
		if($this->tax == '') $this->tax = wp_invoice_meta($invoice_id,'tax_value');


		$this->custom_invoice_id = wp_invoice_meta($invoice_id,'wp_invoice_custom_invoice_id');
		$this->due_date_day = wp_invoice_meta($invoice_id,'wp_invoice_due_date_day');
		$this->due_date_month = wp_invoice_meta($invoice_id,'wp_invoice_due_date_month');
		...
		$this->subscription_start_month = wp_invoice_meta($invoice_id,'wp_invoice_subscription_start_month');
		$this->subscription_start_day = wp_invoice_meta($invoice_id,'wp_invoice_subscription_start_day');
		$this->subscription_start_year = wp_invoice_meta($invoice_id,'wp_invoice_subscription_start_year');
		$this->subscription_total_occurances = wp_invoice_meta($invoice_id,'wp_invoice_subscription_total_occurances');
