...
function wp_invoice_paid_status($invoice_id) {
	...
	global $wpdb;
	if(isset($invoice_id) && wp_invoice_meta($invoice_id,'paid_status') || $wpdb->get_var("SELECT status FROM  ".WP_Invoice::tablename('main')." WHERE invoice_num = '$invoice_id'")) return true;
}
