...
function wp_invoice_calculate_owed() {
		...
		jQuery("#wp_invoice_total_owed").html(jQuery("#invoice_sorter_table tr:visible .column-balance").sum());
...
function wp_invoice_create_username() {
	...
	function wp_invoice_confirm_delete() {
...
jQuery(document).ready(function(){
	...
	jQuery("#wpi_js_conflict_check").hide();
	jQuery("#wp_invoice_billing_information").tabs({cookie:  { expires: 30 }});
	jQuery("#wp_invoice_settings_page").tabs({cookie:  { expires: 30 }});
	
	
	jQuery(".wp_invoice_delete_itemized_row").live('click', function() {
	
		// Don't delete last tor
		var parent =jQuery(this).parent().parent();
		var count = jQuery("#invoice_list tr").length;
	...
	jQuery('#delete_all_wp_invoice_databases').click(function() {
		...
		jQuery.prompt(txt,{	buttons:{Delete:true, Cancel:false}, callback: function(v,m){ if(v){  document.location = "admin.php?page=invoice_settings&wp_invoice_action=complete_removal"; }	}
	});
	return false
	});
	...
	//jQuery('#wp_invoice_main_info .invoice_description').autogrow();
