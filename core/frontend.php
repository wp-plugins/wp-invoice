...
function wp_invoice_the_content($content) {
	...
	$result .= "<div id=\"invoice_page\" class=\"clearfix\">";
		...
		// Show reciept if coming back from PayPal
 		if(isset($_REQUEST['payment_status'])) { $result .= wp_invoice_show_paypal_reciept($invoice_id) ;  $stop = true; }
 		if(isset($_REQUEST['paypal_recurring_return'])) { $result .= wp_invoice_show_paypal_reciept($invoice_id) ;  $stop = true; }
...
#invoice_page #recurring_info {width: 400px; float:left; padding-right: 10px; margin-right: 15px; position: relative;}
