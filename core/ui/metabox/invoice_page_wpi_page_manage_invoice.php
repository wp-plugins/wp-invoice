...
?>
        <input style="width: 80px;" class="input_field wp_invoice_custom_invoice_id<?php if (empty($this_invoice['custom_id'])) {
            echo " wp_invoice_hidden";
        } ?>" name="wpi_invoice[meta][custom_id]" value="<?php echo !empty($this_invoice['custom_id'])?$this_invoice['custom_id']:''; ?>">
        <span class="wp_invoice_custom_invoice_id"><?php echo $this_invoice['invoice_id']; ?></span> <a onClick='jQuery(".wp_invoice_custom_invoice_id").toggle(); return false;' class="wp_invoice_click_me <?php if (!empty($this_invoice['custom_id'])) {
            echo " wp_invoice_hidden";
    ...
    <tr class="column-payment-method-default wpi_not_for_quote">
      ...
      <td>
        ...
        <?php foreach ($this_invoice['billing'] as $key => $payment_option) : ?>
          <?php if (!isset($payment_option['name'])) continue; ?>
        ...
        if(count($this_invoice['billing']) > 1) {
          echo WPI_UI::checkbox("class=wpi_client_change_payment_method&name=wpi_invoice[client_change_payment_method]&value=true&label=Client can change payment option.", $this_invoice['client_change_payment_method']=='on'?true:false);
        }
        ?>
      </td>
    </tr>
    ...
    <tr class="wpi_advanced_payment_options wpi_not_for_quote">
      <td colspan="2">
        <div class="wp_invoice_accordion">
          <?php foreach ($this_invoice['billing'] as $key => $value) : ?>
            <?php if ( empty($this_invoice['default_payment_method']) ) $this_invoice['default_payment_method'] = key($this_invoice['billing']);  ?>
            <?php if (empty($value['name'])) break; ?>
