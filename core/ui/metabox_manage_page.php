...
<table class="form-table">
	...
	<tr class="invoice_main">
		<th>Tax </th>
		<td style="font-size: 1.1em; padding-top:7px;">
			<input type='text' style="width: 35px;"  name="wp_invoice_tax" id="wp_invoice_tax" autocomplete="off" value="<?php echo $ic->tax ?>">%</input>
		</td>
	</tr>
...
<?php if(!empty($ic->invoice_id)): ?>
<div id="delete-action">
<a href="<?php echo admin_url("admin.php?page=web-invoice_page_overview&delete_single_invoice={$ic->invoice_id}&_wpnonce=" . wp_create_nonce('wp_invoice_delete_' . $ic->invoice_id));?>" class="submitdelete deletion">Delete</a>
</div>
<?php endif; ?>
...
<?php }
	...
	<td>
		...
		<?php
		$counter = 1;
		foreach($ic->itemized as $itemized_item){	 ?>
		
		<tr valign="top">
			<td valign="middle"><span class="wp_invoice_delete_itemized_row">Delete</span></td>
			<td valign="top" class="id"><?php echo $counter; ?></td>
			<td valign="top" class="name"><input type="text" class="item_name" name="itemized_list[<?php echo $counter; ?>][name]" value="<?php echo stripslashes($itemized_item[name]); ?>" /></td>
			...
			<td valign="top" class="quantity"><input   type="text" autocomplete="off"  value="<?php echo stripslashes($itemized_item[quantity]); ?>" name="itemized_list[<?php echo $counter; ?>][quantity]" id="qty_item_<?php echo $counter; ?>"  class="item_quantity"></td>
			<td valign="top" class="price"><input  type="text" autocomplete="off" value="<?php echo stripslashes($itemized_item[price]); ?>"  name="itemized_list[<?php echo $counter; ?>][price]" id="price_item_<?php echo $counter; ?>"  class="item_price"></td>
	...
	<tr class="invoice_main">
		...
		<td>
			...
			<tr>
			<td align="right"><span class="wp_invoice_enable_recurring_billing" <?php if(!$ic->is_recurring) { ?>style="display:none;"<?php } ?>><?php _e("Recurring Invoice Total:", WP_INVOICE_TRANS_DOMAIN) ?></span></td>
			<td class="item_total"><span  class="wp_invoice_enable_recurring_billing" <?php if(!$ic->is_recurring) { ?>style="display:none;"<?php } ?> id='recurring_total'></span></td>
			</tr>
	...
	$wp_invoice_paypal_allow 	= ($ic->paypal_allow ? 'yes' : 'no');
	$wp_invoice_paypal_address 	= $ic->paypal_address;
	
	
 	$wp_invoice_cc_allow 				= ($ic->cc_allow ? 'yes' : 'no');
	$wp_invoice_gateway_username 		= $ic->gateway_username;
...
<table class="form-table">
	...
	<tr>
		<th><?php _e("Client can change payment method:") ?></th>
			<td>
			<?php echo wp_invoice_draw_select('wp_invoice_client_change_payment_method',array("1" => __('Yes', WP_INVOICE_TRANS_DOMAIN), "0" => __('No', WP_INVOICE_TRANS_DOMAIN)), $ic->client_change_payment_method); ?>
		</td>
...
<?php }
	...
	function wp_invoice_metabox_recurring_billing($ic) { 
...
<div id="wp_invoice_enable_recurring_billing" class="wp_invoice_click_me" <?php if($ic->is_recurring) { ?>style="display:none;"<?php } ?>>
	<?php _e("Create a recurring billing schedule for this invoice.", WP_INVOICE_TRANS_DOMAIN) ?>
</div>

<div class="wp_invoice_enable_recurring_billing" <?php if(!$ic->is_recurring) { ?>style="display:none;"<?php } ?>>
...
<table class="form-table" id="">
	...
	<tr>
		<th><a class="wp_invoice_tooltip"  title="<?php _e("Start dates only work with Authorize.net, not PayPal.", WP_INVOICE_TRANS_DOMAIN) ?>"><?php _e("Start Date", WP_INVOICE_TRANS_DOMAIN) ?></a></th>
		<td>
			
			
			<span style="<?php if($ic->recurring_auto_start) { ?>display:none;<?php } ?>" class="wp_invoice_timestamp"><?php _e("Start automatically as soon as the customer enters their billing information. ", WP_INVOICE_TRANS_DOMAIN) ?><span class="wp_invoice_click_me" onclick="jQuery('.wp_invoice_timestamp').toggle();"><?php _e("Specify Start Date", WP_INVOICE_TRANS_DOMAIN) ?></span></span>
			<div style="<?php if(!$ic->recurring_auto_start) { ?>display:none;<?php } ?>" class="wp_invoice_timestamp">
...
<table class="form-table" id="">
	...
	<?php if($ic->create_new_user) {
		...
		$user_email =  (!empty($_REQUEST[wp_invoice_userlookup]) ? $_REQUEST[wp_invoice_userlookup] : false);
	...
	<tr>
		...
		<td><?php echo wp_invoice_draw_inputfield('wp_invoice_new_user_email_address', $user_email); ?></td>
	</tr>
	...
	<tr style="height: 90px;">
		...
		<td>
			<div id="wp_invoice_edit_user_from_invoice">
			  ...
			  <span class="wp_invoice_make_editable<?php if(!$ic->country) echo " wp_invoice_unset"; ?>" id="wp_invoice_country"><?php echo ($ic->country ? $ic->country : "Set Country Code"); ?></span>
			</div>
		</td>
	</tr>
	...
	<?php /*
	<tr>
		<th><?php _e("Localization") ?></th>
		<td>
			<select name="wp_invoice_localization">
				<option value="US" <?php if($ic->wpi_localization == 'US') echo "SELECTED"; ?>>United States</option>
				<option value="UK" <?php if($ic->wpi_localization == 'UK') echo "SELECTED"; ?>>United Kingdom</option>
			</select>
		</td>
	</tr>
	*/ ?>
