...
*/
	function wp_invoice_invoice_row($invoice) {
		...
		if($invoice->is_recurring) {
			$columns = get_column_headers("invoices_page_recurring_billing");
			$hidden = get_hidden_columns("invoices_page_recurring_billing");
		} else {
...
function wp_invoice_options_manageInvoice($invoice_id = '',$message='')
{
	...
	require_once WP_INVOICE_UI_PATH . 'metabox_manage_page.php';
	...
	if($invoice_id && wp_invoice_get_invoice_status($invoice_id,'100'))
		add_meta_box('wp_invoice_metabox_history', __("This Invoice's History (<a href='admin.php?page=new_invoice&invoice_id=$invoice_id&wp_invoice_action=clear_log'>Clear Log</a>)",WP_INVOICE_TRANS_DOMAIN), 'wp_invoice_metabox_history', 'invoices_page_new_invoice', 'normal', 'low');

	if(!$invoice_class->user_deleted):
		$wp_invoice_user_metabox_title = ($invoice_class->create_new_user ? "New User Information" : "User Information");
		add_meta_box('wp_invoice_user_metabox', __($wp_invoice_user_metabox_title,WP_INVOICE_TRANS_DOMAIN), 'wp_invoice_user_metabox', 'invoices_page_new_invoice', 'normal', 'high');
	endif;
