  ...
  **/
  ...
  function wpi_get_pdf($invoice) {
    ...
    if ($wpi_settings['pdf']['display_logo'] == 'true') {
      if ( $headers && strstr($content_type, 'image') && filter_var($wpi_settings['pdf']['logo_path'], FILTER_VALIDATE_URL) ) {
        $logo = str_replace('%logo%', $wpi_settings['pdf']['logo_path'], $logo);
      } else {
        $logo = str_replace('%logo%', $_SERVER['DOCUMENT_ROOT'].$wpi_settings['pdf']['logo_path'], $logo);
      }
