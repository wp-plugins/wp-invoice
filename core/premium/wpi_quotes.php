...
class wpi_quotes {
   ...
   */
  function settings_options( $wpi_settings ) {
    ?>
      <tr>
        <th><?php _e('Quotes', WPI);?></th>
        <td>
          <ul>
            <li>
              <?php
                echo WPI_UI::checkbox(array(
                    ...
                    'value' => 'true',
                    'label' => __('Make recipients to be authorized to be able to Accept Quote.', WPI)
                ), $wpi_settings['quotes']['make_authorize'] );
              ?>
              <div class="description"><?php _e('By default quote recipients do not have to sign in before Accepting the quote you sent to them. Tick this option if you need them to be logged in.', WPI);?></div>
            </li>
          </ul>
        </td>
      </tr>
            ...
            <li>
              <?php
                echo WPI_UI::checkbox(array(
                    ...
                    'value' => 'true',
                    'label' => __('Add "Terms &amp; Conditions" checkbox to Quote page.', WPI),
                    'class' => 'wpi_show_advanced'
                ), $wpi_settings['quotes']['tos_checkbox'] );
              ?>
              <div class="description"><?php _e('This option allows you to add "Terms &amp; Conditions" checkbox to every Quote page. Be sure you have specified Terms page ID below.', WPI);?></div>
            </li>
            <li class="wpi_advanced_option">
              <label for=""><?php _e( '"T&amp;C" Page ID:', WPI ); ?></label>
              <?php echo WPI_UI::input(array(
                  'type' => 'text',
                  'style' => 'width:50px;',
                  'name' => 'tos_page_id',
                  'group' => 'wpi_settings[quotes]',
                  'value' => $wpi_settings['quotes']['tos_page_id']
              )); ?>
              <div class="description wpi_advanced_option"><?php _e('Numeric value of WordPress page ID that has "Terms &amp; Conditions".', WPI) ?></div>
            </li>
          </ul>
        </td>
      </tr>
  ...
  function accept_quote_button() {
    ...
    if ( !empty( $wpi_settings['quotes']['tos_checkbox'] )
      ...
      if ( $link ) {
        ...
        <div>
          ...
          <label for="quote_tos"><?php echo sprintf(__( 'I accept <a target="_blank" href="%s">Terms &amp; Conditions</a>' ), $link); ?></label>
        </div>
        <?php
      }
    }
    ...
    ?>
      <form id="wpi_quote_accept" action="" method="">
        ...
        <input type="submit" value="<?php _e( 'Accept Quote', WPI ); ?>" />
          ...
          jQuery('#wpi_quote_accept').on('submit', function(e){
            if ( !jQuery('#quote_tos').length ) {
              ...
              if ( jQuery('#quote_tos').is(":checked") ) {
                ...
                jQuery('.wpi_quote_accept_result', e.target).html('<?php _e('You have to accept Terms &amp; Conditions.', WPI); ?>').addClass('wpi_error').show();
                return false;
              }
            }
          });
  ...
  function accept_quote() {
    ...
    if ( !is_user_logged_in() && !empty( $wpi_settings['quotes']['make_authorize'] ) && $wpi_settings['quotes']['make_authorize'] == "true" )
      die(json_encode(array('success'=>false, 'message'=>__('Authorization required.', WPI))));
    ...
    if ( wp_verify_nonce($_POST['my-nonce'], 'wpi_accept_quote') ) {
      ...
      if ( $current_user->data->user_email != $invoice_object->data['user_email'] && ( !empty( $wpi_settings['quotes']['make_authorize'] ) && $wpi_settings['quotes']['make_authorize'] == "true" ) ) {
        die( json_encode(array('success'=>false, 'message'=>__('You are not allowed to make that action. You are not a recipient of this quote. Plese login under proper account.', WPI))) );
      }
      ...
      die(json_encode(array('success'=>true, 'message'=>__('Quote has been accepted and converted to invoice. Refreshing...', WPI))));
    }
    die(json_encode(array('success'=>false, 'message'=>__('Error! Unknown request.', WPI))));
  }
