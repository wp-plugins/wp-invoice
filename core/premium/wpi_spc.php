...
class wpi_spc {
  ...
  function wpi_premium_loaded() {
    ...
    $wpi_checkout['info_block']['billing_information'] = array(
      'cc_number' => array(
        'label' => __('Credit Card', WPI),
        'type' => 'input'
      ),
      'cc_expiration' => array(
        'label' => __('Expiration Date', WPI),
        'type' => 'input'
      ),
      'cc_code' => array(
  ...
  function template_redirect() {
    ...
    if ( $wpi_settings['spc_checkout']['enforce_ssl'] == 'true' && (!isset($_SERVER['HTTPS']) || $_SERVER['HTTPS'] != "on") ) {
    ...
    if ( $wpi_settings['spc_checkout']['enforce_ssl'] == 'false' && !empty( $_SERVER['HTTPS'] ) ) {
      if (preg_match('/\[wpi_checkout.*?\]/', $post->post_content)) {
   ...
   * @param type $post
   */
  function paypal_complete_ipn( $post ) {
          ...
          foreach ($custom_params as $param) {
            if ( $param[0] == 'cbf' && function_exists( $param[1] ) ) {
              ...
              $transaction_data = array(
                ...
                'items' => $items,
                'other_meta' => array(
                  'charge_amount' => $post['payment_gross'],
	...
	function available_gateways() {
		...
		foreach ( $wpi_settings['billing'] as $key => $value ) {
			if ( $value['allow'] == 'true' && in_array( $key, self::$accept_gateways ) ) {
	 ...
	 * @author korotkov@ud
	 */
	function remove_abandoned_transactions() {
	...
	function cron_schedules( $schedules ) {
		...
		$schedules['ten_minutes'] = array(
			'interval' => 600,
			'display' => __('Every 10 Minutes', WPI)
		);
		return $schedules;
	}
   ...
   * @return string
   */
  function paid_message( $current_message, $invoice ) {
    if ( $invoice->data['type'] == 'single_payment' ) return __('Transaction complete.', WPI);
   ...
   * @author korotkov@ud
   */
  function contextual_help( $data ) {

    $data['Main'][] = "<p>".__('<b>Single Page Checkout</b><br /> Main settings for Single Page Checkout Premium Feature. <a target="_blank" href="https://usabilitydynamics.com/tutorials/wp-invoice/single-page-checkout/">More...</a>')."</p>";

    $data['Single Page Checkout'][] = "<h3>".__('Single Page Checkout')."</h3>";
    $data['Single Page Checkout'][] = "<p>".__('Create single page checkout forms that are then visible in the "Invoice" overview.', WPI)."</p>";
    $data['Single Page Checkout'][] = "<p>".__('You can create Single Checkout Page by using shortcode <b>[wpi_checkout]</b>. Options:', WPI)."</p>";

    $options['title']                = __('The title that will be displayed in control panel overview.', WPI);
    $options['terms_page_id']        = __('ID of page that has sales terms, inserts a link into SPC to the page.', WPI);
    $options['item']                 = __('A comma separated list of items that the SPC is for.', WPI);
    $options['callback_function']    = __('PHP function to be executed when transaction is complete.', WPI);
    $options['customer_information'] = __('A CSV list of Title of any custom attributes for the checkout.', WPI);
    $options['custom_amount']        = __('`true/false` value. If true then custom amount field will appear on SPC. Do not use <u>fee</u> and <u>items</u> in case of custom_amount is true.', WPI);
    $options['uncheck_items']        = __('Disabled the checkout items, only use this if you are using some sort of JS script to enable them back on.', WPI);
    $options['fee']                  = __('Percent value of fee applied to items amounts (ex. fee=2 or fee="2%" )', WPI);
    $options['gateways']             = __('CSV list of gateways available for payment.', WPI).__(' Available values: ').'<b>'.self::gateways_values().'</b>';
    ...
    $data['Single Page Checkout'][] = "<p>".__('Example: <b>[wpi_checkout title="Water Bill Payment" custom_amount="true" fee="5%"]</b>', WPI)."</p>";
   ...
   * @return mixed
   */
  function custom_field_save( $args ) {
   ...
   * @return number
   */
  function process_custom_amount( $total, $wpi_checkout ) {
   ...
   *
   */
  function wpi_checkout_process( $data = false ) {
    ...
    /* Validate Request */
    if($data['wpi_checkout']['terms'] == 'false') {
      $return['payment_status'] = 'validation_fail';
      $return['missing_data']['terms'] = __('Terms must be accepted.', WPI);
    }

    if(empty($billing['user_email'])) {
      $return['payment_status'] = 'validation_fail';
      $return['missing_data']['user_email'] = __('An e-mail address is required.', WPI);
    }
    if(empty($billing['first_name'])) {
      $return['payment_status'] = 'validation_fail';
      $return['missing_data']['first_name'] = __('You must enter a name.', WPI);
    }
    if(empty($billing['cc_number']) && $data['wpi_checkout']['payment_method'] == 'wpi_authorize') {
      $return['payment_status'] = 'validation_fail';
      $return['missing_data']['cc_number'] = __('You must enter a valid credit card number.', WPI);
    }
    if(empty($billing['cc_expiration']) && $data['wpi_checkout']['payment_method'] == 'wpi_authorize') {
      $return['payment_status'] = 'validation_fail';
      $return['missing_data']['cc_expiration'] = __('You must enter an expiration date.', WPI);
    }
    ...
    $process_data = array(
      ...
      'payer_first_name' => $billing['first_name'],
      'payer_last_name' => $billing['last_name'],
      'payer_email' => $billing['user_email'],
      'cc_number' => $billing['cc_number'],
      'cc_expiration' => $billing['cc_expiration'],
      'cc_code' => $billing['cc_code'],
      ...
      'zip' => $billing['zip'],
      'currency_code' => $data['wpi_checkout']['currency_code']
    );
     ...
     */
    $process_data = apply_filters( 'wpi_spc_pre_process_data', $process_data );
    ...
    if($result['payment_status'] == 'Complete') {
      ...
      if($invoice_id = self::save_invoice($transaction_data)) {
        ...
        $return['message'] = apply_filters("wpi_checkout_payment_success_message",  __('Thank you. Your payment has been processed.', WPI), $items, $result);
      ...
      $return['message'] = !empty( $result['error_message'] ) ? __('Transaction Error: ', WPI) . $result['error_message'] :  __('There was an error with the transaction. Please select another payment venue, or contact support.', WPI);
    }
   ...
   */
  function save_invoice($transaction_data) {
    global $wpi_checkout;
    ...
    $items = implode(',', $items);
		...
		if ( empty( $items ) && !empty( $transaction_data['amount'] ) ) {
			$description = __('Custom Amount: ' . wp_invoice_currency_format( (float)$transaction_data['amount'] ), WPI);
		} else {
			$description = __("Items: ", WPI) . $items;
		}
		if ( !empty( $transaction_data['fee'] ) ) {
			$description .= " + " . wp_invoice_currency_format( (float)$transaction_data['fee'] ) . __(" of fee.", WPI);
		}

    $subject = __("Transaction #", WPI) . $transaction_data['other_meta']['transaction_id'] . " (". $transaction_data['other_meta']['cc_number'] .")";
    if ( !empty( $transaction_data['title'] ) ) {
      $subject = $transaction_data['title']." (".__('Transaction #', WPI) . $transaction_data['other_meta']['transaction_id'] . ")";
    }
    ...
    if ( !empty( $transaction_data['title'] ) ) {
      ...
      $invoice->set(array(
        "created_by" => __("Default Checkout Form", WPI)
      ));
    }
    ...
    if ( !empty( $transaction_data['amount'] ) ) {
      $invoice->line_item(array(
          "name" => __('Amount', WPI),
          "description" => __('Custom amount from SPC', WPI),
          "quantity" => 1,
		...
		if ( $transaction_data['transaction']['payment_method'] == 'wpi_paypal' ) {
			/** Set invoice as pending if PayPal */
			$invoice->set(array(
        "post_status" => "pending"
      ));
		}
    ...
    if(strtolower($transaction_data['transaction']['payment_status']) == 'complete') {
      if (!empty($invoice_id)) {
        ...
        /* Add payment */
        $note = sprintf(__("Transaction #%s was successfully completed.", WPI), $transaction_data['transaction']['transaction_id']);
        $event_type = 'add_payment';
        $event_amount = $transaction_data['other_meta']['charge_amount'];
        $event_note = WPI_Functions::currency_format(abs($event_amount), $invoice_id)." ".__('paid in', WPI)." - $note";
        $timestamp = time();
        ...
        /* If transaction was not saved we add Error log with transaction information */
        $log = __("Transaction #", WPI) . $transaction_data['transaction']['transaction_id'] . " (". $transaction_data['other_meta']['cc_number'] .")";
        $log .= __(" was successfully completed, but invoice of the current transaction could not be saved. We advise You to add transaction's invoice manually.", WPI);
        $log .= __(" Charge Amount: ", WPI) . $transaction_data['other_meta']['charge_amount'] . ". ".__('Items of Transaction:', WPI). " ". $items;
        WPI_Functions::log($log);
      }
    }
    return false;
  }
   ...
   *
   */
  function shortcode_wpi_checkout( $atts = "",  $content = null, $code = "") {
    ...
    $atts = wp_parse_args( $atts, array(
      'terms_page_id'     => '',
      'terms_text'        => __( 'I accept the <a href="%s" target="_blank">terms and conditions</a>.', WPI ),
      'return'            => 'true',
      'uncheck_items'     => 'false',
      'hidden_attributes' => '',
      'template'          => '',
    ...
    /*if(!$item_found) {
      return '<div class="wpi_checkout wpi_requested_item_not_found">' . __('Requested checkout item was not found.') . '</div>';
    }*/
    ...
    foreach( $gateways as $gateway ) {
      if ( array_key_exists( $gateway, $atts[ 'env_vars' ][ 'system_gateways' ] ) && is_array( $atts[ 'env_vars' ][ 'system_gateways' ][$gateway] ) ) {
        ...
        $atts[ 'env_vars' ][ 'available_gateways' ][$gateway][ 'data' ] = apply_filters( "wpi::checkout::{$gateway}::script", array(
          ...
          'strings' => array(
            'processing' => __( 'Processing...', WPI ),
            'process_payment' => __( 'Process Payment', WPI ),
            'hacking' => __( 'Hacking?', WPI ),
            'redirecting_to_paypal' => __( 'Redirecting to PayPal. Wait please...', WPI ),
          )
        ) );
      }
    }
    ...
    $atts[ 'env_vars' ][ 'ga_event_tracking' ] = $wpi_settings['ga_event_tracking'];
    ...
    $template_found = UD_API::get_template_part( array(
      ...
      "wpi_checkout"
    ), apply_filters( 'wpi::spc::template', array( WPI_Templates_Path ) ) );
    ...
    if( !empty( $available_gateways ) && is_array( $available_gateways ) ) {
      foreach( $available_gateways as $k => $d ) {
        wp_enqueue_script( "wpi.checkout.{$k}", WPI_Gateways_URL . "/js/{$k}_checkout.js", array( 'wpi.checkout' ) );
      }
    }
   ...
   * @return array
   */
  function add_custom_amount_field( $current ) {
   ...
   * @author korotkov@ud
   */
  function process_inline_fee( $total, $atts ) {
    $fee = (int)$atts['fee'];
   ...
   * @return md5 hash
   */
  function generate_security_hash( $fee, $total ) {
  ...
  function wpi_object_types($types) {
    $types['single_payment'] = array('label' => __('Single Payment', WPI));
    return $types;
  }


  /**
   * Content to display inside the Checkout section on the  Reports page
   *
   * @since 1.0
   * @TODO: Report data needs to be implemented here. Maxim Peshkov.
   */
  function report_page_tab_content($wpi_settings) {
    ?>
    <h3 class="section_header"><?php _e('Sales Report Data', WPI); ?></h3>
    <?php
  }
   ...
   *
   */
  function settings_page_content($wpi_settings) {
    ?>
      <tr>
        <th><?php _e('Single Page Checkout', WPI);?></th>
        <td>
          <ul>
            <li>
              <?php echo WPI_UI::checkbox("&name=wpi_settings[spc_checkout][enforce_ssl]&value=true&label=" . __('Enforce SSL on checkout pages.', WPI),$wpi_settings['spc_checkout']['enforce_ssl'])?>
              <div class="description"><?php _e('As for regular invoice pages, HTTPS should be enforced on pages that have a checkout form.', WPI);?></div>
            </li>
          </ul>
        </td>
      </tr>
   ...
   * @param type $attrs
   */
  function hidden_attributes( $attrs ) {
   ...
   * @param type $post_id
   */
  function after_save_action( $post_id ) {
    $data = array();
    parse_str($_REQUEST['data'], $data);
   ...
   * @return type
   */
  function set_fixed_user_email( $current, $slug, $attrs ) {
    if ( $slug == 'user_email' ) {
      return $attrs[$slug];
    }
    return $current;
   ...
   * @return string
   */
  function set_user_email_readonly( $input_attributes, $slug ) {
    if ( $slug == 'user_email' ) {
...
// Initiate feature
add_action('wpi_premium_loaded', array('wpi_spc', 'wpi_premium_loaded'));