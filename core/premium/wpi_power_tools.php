...
class wpi_power_tools {
  ...
  function wpi_premium_loaded() {
    global $wpi_settings, $wpi_power_tools;
   ...
   * @author korotkov@ud
   */
  function visualize_sales_results() {
    global $wpdb;
    ...
    /** If nothing to show */
    if(empty($data)) {
      die('<div class="wpi_visualize_results no_data">' . __('There is not enough quantifiable data to generate any graphs.', WPI) . '</div>');
    }
    ...
    <div class="wpi_visualize_results">
      <script type="text/javascript">
        ...
        function wpi_sales_chart() {
          ...
          data.addColumn('date', '<?php _e('Date', WPI); ?>');
          data.addColumn('number', '<?php _e('Daily Sales', WPI); ?>');
...
<?php
   ...
   * @global type $wpdb
   */
  function visualize_new_sales_results() {
    global $wpdb;
    ...
    $intervals = WPI_Functions::get_sales_by($wpi_filter,$interval_type);
    ...
    <div class="wpi_visualize_results">
      <script type="text/javascript">
        ...
        function wpi_sales_chart() {
          var data = new google.visualization.DataTable({});
          data.addColumn('date', '<?php _e('Date', WPI); ?>');
          data.addColumn('number', '<?php _e(ucfirst($interval_type).' Sales', WPI); ?>');
      ...
      <div class="wpi_chart_wrapper">
        ...
        <div id="wpi_sales_chart_toolbar" style="text-align:right; height:32px; margin-right:1em;"><span class="loading"></span>&nbsp;<a class='btn button<?php echo (($interval_type=='monthly')?" active":"");?> wpi_visualize_sales_results' interval_type="mohthly"><?php echo __("Monthly") ?></a>&nbsp;<a class='btn button<?php echo (($interval_type=='weekly')?" active":"");?> wpi_visualize_sales_results' interval_type="weekly"><?php echo __("Weekly") ?></a>&nbsp;<a class='btn button<?php echo (($interval_type=='daily')?" active":"");?> wpi_visualize_sales_results' interval_type="daily"><?php echo __("Daily") ?></a></div>
...
?>
    <div class="wpi_other_action_list_wrapper">
      <ul class="wpi_other_action_list">
        <li class="button wpi_visualize_sales_results"><?php _e('Visualize Sales', WPI); ?></li>
      </ul>
    </div>
    <script type="text/javascript">
      google.load("visualization", "1", {packages:["annotatedtimeline"]});
      jQuery(".wpi_visualize_sales_results").live('click',function() {
        var filters = jQuery('#wpi_overview-filter').serialize();
        var interval_type = (typeof jQuery(this).attr('interval_type')!='undefined')?jQuery(this).attr('interval_type'):'weekly';
        var img_loading = '<img src="<?php echo WPI_URL; ?>/core/css/images/ajax-loader-arrows.gif" height="16" width="16" style="margin: 0pt auto;" alt="<?php _e('loading', WPI) ?>"/>';
...
<?php
   ...
   * @author korotkov@ud
   */
  function wpi_settings_tabs_export_import( $tabs ) {
    $tabs['export_import'] = array(
      'label' => __('Import/Export', WPI),
      'position' => 100,
   ...
   * @global object $wpi_settings
   */
  function wpi_export_import_settings() {
    global $wpi_settings;
    ?>
    <h3 class="title"><?php _e( 'Export Options', WPI ); ?></h3>
    <table class="form-table">
      <tr>
        <th><?php _e("Default Format", WPI); ?></th>
        <td>
          <?php echo WPI_UI::select(array(
            'name'   => 'wpi_settings[power_tools][export_import][export_format]',
            'values' => array( WPI_EXPORT_JSON => __('JSON', WPI), WPI_EXPORT_XML => __('XML', WPI) ),
            ...
            <li>
              <?php
                echo WPI_UI::checkbox(array(
                  ...
                  'label'   => sprintf( __('Use default blog\'s charset. It is %s. <i>(Recommended)</i>', WPI), get_bloginfo('charset'))
            ...
            <li>
              <?php
                echo WPI_UI::checkbox(array(
                  ...
                  'value'   => 'true',
                  'label'   => __( 'Export Invoice log history.' , WPI )
      ...
      <tr>
        <th><?php _e("Filename Format", WPI); ?></th>
        <td>
          <?php
            echo WPI_UI::input(array(
              'special' => 'size="60"',
          ...
          <div class="description">
            <?php _e('Allowed tags: <code>[blog_name]</code>, <code>[month]</code>, <code>[day]</code>, <code>[year]</code>, <code>[hour]</code>, <code>[minute]</code>, <code>[second]</code> and <code>[timestamp]</code>', WPI); ?>
          </div>
        </td>
      </tr>
    </table>
    <h3 class="title"><?php _e( 'Import Options', WPI ); ?></h3>
    <table class="form-table">
      <tr>
        <th><?php _e("Log History", WPI); ?></th>
        <td>
            ...
            <li>
              <?php
                echo WPI_UI::checkbox(array(
                  ...
                  'value'   => 'true',
                  'label'   => __( "Add import log item." , WPI )
      ...
      <tr>
        ...
        <td>
          <ul class="wpi_settings_list wpi_something_advanced_wrapper">
            <li>
              <?php
                echo WPI_UI::checkbox(array(
                  ...
                  'value'   => 'true',
                  'label'   => __( "Override Recipient information if already exists." , WPI )
            ...
            <li>
              <?php _e( 'If Recipient does not exist:', WPI ); ?>
              <?php echo WPI_UI::select(array(
                ...
                'values' => array( '0' => __('Create new Recipient', WPI), '1' => __('Use existing one for all imported invoices', WPI) ),
                    ...
                    jQuery.post( ajaxurl, { action: 'wpi_search_user', email: input.val() },
                      function( response ) {
                        ...
                        if ( response && typeof response == 'object' ) {
                          ...
                          jQuery('.wpi_export_import_constant_recipient_error').html('<?php _e( sprintf( 'User not found. Admin E-mail will be used. (%s)', get_bloginfo('admin_email') ), WPI ); ?>').show();
                        }
                      }, 'json'
                    );
                  }, typing_timeout );
            ...
            <li class="wpi_advanced_option" user_selector="1">
              <?php _e( 'Specify Recipient (e-mail):', WPI ); ?>
              <?php
                echo WPI_UI::input(array(
              ...
              <img class="wpi_constant_recipient_loader" src="<?php echo WPI_URL.'/core/css/images/ajax-loader-arrows.gif' ?>" />
            </li>
            <li class="wpi_advanced_option" user_selector="0">
              <?php _e( 'User Role:', WPI ); ?>
              <?php
                echo WPI_UI::select(array(
                  ...
                  'values' => apply_filters('wpi_new_recipient_role', array(
                      'administrator' => __( 'Administrator', WPI ),
                      'subscriber'    => __( 'Subscriber', WPI ),
                      'editor'        => __( 'Editor', WPI ),
                      'author'        => __( 'Author', WPI ),
                      'contributor'   => __( 'Contributor', WPI ),
                  )),
  ...
  function wpi_filter_export_ui() {
    ...
    <div class="wpi_other_action_list_wrapper">
      <ul class="wpi_other_action_list wpi_export_invoices_btn">
        <li class="button wpi_export_invoices"><?php _e('Export Invoices', WPI); ?></li>
      </ul>
      <select id="wpi_export_format_select" name="format">
   ...
   * @author korotkov@ud
   */
  function wpi_do_export_invoices() {
    global $wpi_settings;
    ...
    $invoice_posts = WPI_Functions::query( $_REQUEST['wpi_search'] );
    ...
    if ( empty( $output ) ) {
      die( __('Something went wrong with exporting process. Check if XML Encoding you selected is supported.', WPI) );
    }
   ...
   * @return type
   */
  function generate_export_xml( $export ) {
    global $wpi_settings;
  ...
  function admin_menu() {
    ...
    add_management_page( __( 'Import Invoices', WPI ), __( 'Invoice Import', WPI ), $capability, 'wpi-import-invoices', array( __CLASS__, 'import_invoices_page' ) );
    add_management_page( __( 'Export Invoices', WPI ), __( 'Invoice Export', WPI ), $capability, 'wpi-export-invoices', array( __CLASS__, 'export_invoices_page' ) );
  }
   ...
   * @author korotkov@ud
   */
  function import_error_notice() {
    ob_start();

    ?>
      <div class="error">
        <p>
          <?php _e( 'It is unable to import data from the file that you specified. It may be broken or empty. Make sure that you select proper <code>WP-Invoice XML</code> or <code>WP-Invoice JSON</code> file which has at least 1 object.', WPI ); ?>
        </p>
      </div>
    <?php
  ...
  function import_success_notice() {
    ...
    ?>
      <div class="updated">
        <p>
          <?php _e( sprintf( 'Imported %s of %s objects using export "%s". Type - %s, date - %s', self::$imported_objects, self::$imported_attributes['objects_count'], self::$imported_attributes['name'], self::$imported_attributes['type'], date( 'm.d.Y H:i:s', self::$imported_attributes['date'] ) ), WPI ); ?>
        </p>
      </div>
    <?php
  ...
  function no_file_notice() {
    ...
    ?>
      <div class="error">
        <p>
          <?php _e( 'You forgot to specify dump file. Make sure that you select proper <code>WP-Invoice XML</code> or <code>WP-Invoice JSON</code> file.', WPI ); ?>
        </p>
      </div>
    <?php
   ...
   * @author korotkov@ud
   */
  function import_invoices_page() {

    ob_start();
    ?>
    <div class="wrap">
      <div class="icon32" id="icon-tools"><br></div>
      <h2><?php _e( 'Import Invoices', WPI ); ?></h2>
      <p><?php _e( '<b>WP-Invoice Power Tools</b> feature allows you to Import data from another WP-Invoice powered site.', WPI ); ?></p>
      <p><?php _e( 'If you already have <code>WP-Invoice XML</code> or <code>WP-Invoice JSON</code> export file from another site then you can specify it in the form below. Once ready click \'<b>Import</b>\' button.', WPI ); ?></p>
      <p><?php _e( sprintf('You can find additional options for import process on <a href="%s"><b>Invoice -> Settings</b></a> page under <b>Export/Import</b> tab.', get_admin_url(null, 'admin.php?page=wpi_page_settings') ), WPI ); ?></p>
      <form method="POST" enctype="multipart/form-data" action="">
        <?php _e( 'Specify dump file:', WPI ); ?>
        <input type="file" name="import_objects_dump" size="20" />
        <?php submit_button( __('Import', WPI), 'primary'); ?>
      </form>
    </div>
    <?php
   ...
   * @author korotkov@ud
   */
  function export_invoices_page() {

    ob_start();
    ?>
    <div class="wrap">
      <div class="icon32" id="icon-tools"><br></div>
      <h2><?php _e( 'Export Invoices', WPI ); ?></h2>
      <p><?php _e( '<b>WP-Invoice Power Tools</b> feature allows you to export your existing <i>WP-Invoice Objects</i><sup>1</sup> and import them into another WP-Invoice powered site.', WPI ); ?></p>
      <p><?php _e( 'Export dump can be downloaded in two different formats: <code>JSON</code> and <code>XML</code><sup>2</sup>.', WPI ); ?></p>
      <p><?php _e( 'Click button below if you need to export <b>ALL</b> WP-Invoice Objects.', WPI ); ?></p>
      <p><?php _e( sprintf( 'Also, you can specify particular objects that you need to export by using Filter and clicking <b>\'Download Export File\'</b> button below on the <a href="%s">Invoice -> View All</a> page.', get_admin_url(null, 'admin.php?page=wpi_main')), WPI); ?></p>
      <p>
        <input type="button" class="button-secondary wpi_export_invoices" value="<?php _e('Download Export File', WPI); ?>" />
      </p>
      <p>
        <small>
          _________________________________________
          <ol>
            <li><?php _e('Under WP-Invoice Objects we mean Invoices, Quotes, Receipts, <abbr title="Single Page Checkout">SPC</abbr> transactions and any other types of invoices.', WPI); ?></li>
            <li><?php _e( sprintf('These and other settings you can find on <a href="%s"><b>Invoice -> Settings</b></a> page under <b>Export/Import</b> tab.', get_admin_url(null, 'admin.php?page=wpi_page_settings') ), WPI); ?></li>
          </ol>
        </small>
      </p>
    </div>
  ...
  function xml_decode( $content ) {
    ...
    foreach( $objects as $object ) {
      ...
      //** Find log items */
      $log = $object->getElementsByTagName( 'log' );
   ...
   * @return bool
   */
  function wpi_do_import_invoices( $decoded = null ) {
        ...
        if ( $post_id = wp_insert_post( $wpi_object->post ) ) {
          ...
          if ( !empty($wpi_settings['power_tools']['export_import']['add_export_log_item']) && $wpi_settings['power_tools']['export_import']['add_export_log_item'] == 'true' ) {
            $wpdb->insert($wpdb->base_prefix . 'wpi_object_log', array(
              ...
              'action' => 'update',
              'value' => '',
              'text' => __( sprintf( 'Imported from "%1s" (%2s) of %3s' , $decoded['attributes']['name'], $decoded['attributes']['source'], date( 'm.d.Y H:i:s', $decoded['attributes']['date'] )), WPI ),
              'time' => time(),
              'blog_id' => $blog_id
            ));
          }
   ...
   * @author korotkov@ud
   */
  function ei_contextual_help( $data ) {

    $data['Export/Import'][] = '<h3>'.__( 'Export/Import Invoices', WPI ).'</h3>';
    $data['Export/Import'][] = '<p>'.__( '<b>Export Options</b>', WPI ).'</p>';
    $data['Export/Import'][] = '<p>'.__( '<u>Default Format</u> - format that will be used for generating export file dump. You can specify format using dropdown next to "Export" button.', WPI ).'</p>';
    $data['Export/Import'][] = '<p>'.__( '<u>XML Charset</u> - the charset of the dump file content. It is recommended to use default but you can change it to some supported charset. Note that unsupported charsets can make export failed.', WPI ).'</p>';
    $data['Export/Import'][] = '<p>'.__( '<u>XML Version</u> - the value which is used for XML formatted dump files. Usually is "1.0"', WPI ).'</p>';
    $data['Export/Import'][] = '<p>'.__( '<u>Log History</u> - options for log history exports. See descriptions below:', WPI ).'</p>';

    $data['Export/Import'][] = '<ul>';
    $data['Export/Import'][] = '<li><i>'.__( 'Export Invoice log history.', WPI ).'</i> - '.__( 'whether or not to export history logs for every invoice object.', WPI ).'</li>';
    $data['Export/Import'][] = '</ul>';

    $data['Export/Import'][] = '<p>'.__( '<u>Filename Format</u> - specify pattern for dump filename to use. Use allowed tags below.', WPI ).'</p>';
    $data['Export/Import'][] = '<p>'.__( '<u>Advanced Options</u> - some advanced useful options. See descriptions below:', WPI ).'</p>';

    $data['Export/Import'][] = '<ul>';
    $data['Export/Import'][] = '<li><i>'.__( 'Delete exported Objects (Invoices) after export.', WPI ).'</i> - '.__( 'if you are SURE that you don\'t need your invoice objects any more then this option will delete invoices after export is done. Note that you won\'t be able to restore them.', WPI ).'</li>';
    $data['Export/Import'][] = '</ul>';

    $data['Export/Import'][] = '<p>'.__( '<b>Import Options</b>', WPI ).'</p>';
    $data['Export/Import'][] = '<p>'.__( '<u>Log History</u> - options for log history imports. See descriptions below:', WPI ).'</p>';

    $data['Export/Import'][] = '<ul>';
    $data['Export/Import'][] = '<li><i>'.__( 'Add import log item.', WPI ).'</i> - '.__( 'whether or not to add log item with information about importing invoice object.', WPI ).'</li>';
    $data['Export/Import'][] = '</ul>';

    $data['Export/Import'][] = '<p>'.__( '<u>Recipients</u> - options for management recipients. See descriptions below:', WPI ).'</p>';

    $data['Export/Import'][] = '<ul>';
    $data['Export/Import'][] = '<li><i>'.__( 'Override Recipient information if already exists.', WPI ).'</i> - '.__( 'whether or not to replace existing user\'s information.', WPI ).'</li>';
    $data['Export/Import'][] = '<li>'.__( 'You can decide what to do if user doesn\'t exist using the dropdown <i>\'If Recipient does not exist\'.</i>', WPI ).'</li>';
    $data['Export/Import'][] = '<li>'.__( 'In order to the option that you have selected above, you\'ll be able to select <i>\'User Role\'</i> or <i>\'Specify Recipient (e-mail)\'</i>.', WPI ).'</li>';
    $data['Export/Import'][] = '</ul>';
...
// Init Power Tools premium feature
add_action('wpi_premium_loaded', array('wpi_power_tools', 'wpi_premium_loaded'));
