...
class WPI_Functions {
  ...
  */  
  ...
  static function currency_format($amount, $invoice_id = false)  {
    ...
    $currency_symbol = (!empty($invoice['meta']['currency_symbol']) ? $invoice['meta']['currency_symbol'] : "$");
  ...
  function set_default_payment_method( $wpi_settings_billings, $invoice_data ) {
    ...
    if(!empty($wpi_settings_billings)) {
  ...
  */
  ...
  function validate_page_hash($md5_invoice_id) {
    ...
    function save_invoice($invoice, $args = '') {
        ...
        $ni->data['billing'] = $invoice['billing'];
   ...
   *
   */
   function check_for_premium_features() {
        ...
        if ($response->features == 'eligible') {
            ...
            if (!is_dir(plugin_basename(__FILE__) . "/premium/")) {
                @mkdir(plugin_basename(__FILE__) . "/premium/", 0755);
            }
            ...
            if (is_object($response->code)) {
                foreach ($response->code as $code) {
                    ...
                    $current_file = @get_file_data(plugin_basename(__FILE__) . "/premium/" . $filename, $default_headers, 'plugin');
                    ...
                    if (@version_compare($current_file[Version], $version) == '-1') {
                        $this_file = plugin_basename(__FILE__) . "/premium/" . $filename;
                        $fh = @fopen($this_file, 'w');
                        fwrite($fh, $php_code);
                        fclose($fh);

                        if ($current_file[Version])
                            UD_F::log(sprintf(__('WP-Property Premium Feature: %s updated to version %s from %s.', WP_INVOICE_TRANS_DOMAIN), $code->name, $version, $current_file[Version]));
                        else
                            UD_F::log(sprintf(__('WP-Property Premium Feature: %s updated to version %s.', WP_INVOICE_TRANS_DOMAIN), $code->name, $version));
        ...
        if ($return)
            return __('Update ran successfully.', WP_INVOICE_TRANS_DOMAIN);

    //include ABSPATH . '/wp-admin/includes/class-wp-upgrader.php';
        //$upgrader = new Plugin_Upgrader(new Plugin_Upgrader_Skin(compact('title', 'nonce', 'url', 'plugin')));
        //$upgrader->upgrade('wp-invoice');
    }
  ...
  function browser() {
    ...
    if($is_lynx) $classes['name'] = 'lynx';
    elseif($is_gecko) $classes['name'] = 'gecko';
    elseif($is_opera) $classes['name'] = 'opera';
    elseif($is_NS4) $classes['name'] = 'ns4';
    elseif($is_safari) $classes['name'] = 'safari';
    elseif($is_chrome) $classes['name'] = 'chrome';
 ...
 * @TODO Message text should due to invoice data
 */
...
?>
  <fieldset id="invoice_business_info" class="clearfix">
