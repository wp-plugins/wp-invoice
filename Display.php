...
?>
	...
	<select id="wp_invoice_action" name="wp_invoice_action">
		...
		<option value="send_reminder" name="remindid" >Send Reminder(s)</option>
...
function wp_invoice_options_manageInvoice($invoice_id = '',$message='')
{
	...
	echo $info->value . " on <span class='wp_invoice_tamp_stamp'>" . $info->time_stamp . "</span><br />";
	} ?>
	<?php } ?>
...
<table class="form-table" >
<tr>
<td><h2>WP-Invoice Database Tables</h2>
...
function wp_invoice_show_email($invoice_id,$reminder=false) {
	$invoice_info = new WP_Invoice_GetInfo($invoice_id);
...
//	$currency_code = wp_invoice_determine_currency($invoice_id);
	...
	$message = "Dear ". $recipient->recipient('callsign') . ", \n\n";
 	if($reminder) { $message .= get_option("wp_invoice_reminder_message") . " \n\n"; }
